...
const handleIncidentsClick = async () => {
  const pd = initPDJS();
  const { data } = await pd.get("/incidents?limit=200&include[]=users");
...
document.getElementById("csv-file-input").onchange = function () {
  ...
  Papa.parse(this.files[0], {
    ...
    complete: function (results) {
      ...
      tableColumnObjects = tableColumnNames.map((name) => {
        return { data: name, title: userTableColumnsMap[name] };
      });
